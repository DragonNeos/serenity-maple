...
namespace Serenity
{
    public class Session
    {
        ...
        private static void DoAction(Action<DateTime> pAction)
        {
            pAction(DateTime.Now);
        }
        ...
        public Session(Socket pSocket, string tn = "")
        {
            ...
            StartReading(4, true);
            //SwitchAESByVersion(Constants.MajorVersion);
        }
        ...
        public void Connect()
        {
            ...
            try
            {
                ...
                Debug.WriteLine(TypeName + " Connected with server!");
                Disconnected = false;
                StartReading(2, true);
            }
            catch (Exception ex)
        ...
        private void ContinueReading()
        {
            ...
            {
                Debug.WriteLine(TypeName + " [ERROR] ContinueReading(): {0}", ex.ToString());
        ...
        private void EndReading(IAsyncResult pIAR)
        {
            ...
            {
                Debug.WriteLine(TypeName + " Socket Exception while receiving data: {0}", socketException.Message);
                OnDisconnectINTERNAL();
            }
            catch (Exception ex)
            {
                Debug.WriteLine(TypeName + " [ERROR] EndReading(): {0}", ex.ToString());
                OnDisconnectINTERNAL();
            }
        }
        ...
        public void SendData(byte[] pData)
        {
            ...
            try
            {
                ...
                {
                    Debug.WriteLine(TypeName + " [ERROR] Error while sending data: not all bytes transferred: only {0} of {1} sent!", sent, pData.Length);
                }
            }
            catch (Exception ex)
            {
                Debug.WriteLine(TypeName + " [ERROR] Failed sending: {0}", ex.ToString());
                OnDisconnectINTERNAL();
            }
        }
        ...
        private static void SwitchAESByVersion(ushort pVersion)
        {
            _rijndaelAES.Key = Serenity.Connection.GMSKeys.GetKeyForVersion(pVersion);
            _rijndaelAES.Mode = CipherMode.CTS;
            _rijndaelAES.Padding = PaddingMode.PKCS7;
            // ECB: PKCS7, Zeros, None, ISO10126, ANSIX923
            // CBC: PKCS7, Zeros, None, ISO10126, ANSIX923
            // CFB: PKCS7, Zeros, None, ISO10126, ANSIX923
            // CTS: PKCS7, Zeros, None, ISO10126, ANSIX923
        ...
        private static void AESTransform(byte[] pData, byte[] pIV)
        {
            if (_transformer == null)
            {
                ...
                _rijndaelAES.Padding = PaddingMode.ANSIX923;
                _transformer = _rijndaelAES.CreateEncryptor();
            }
